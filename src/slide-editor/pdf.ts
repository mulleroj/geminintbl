...
export async function processPdf(file: File, onProgress: (progress: ProcessProgress) => void): Promise<SlideModel[]> {
  ...
  try {
    ...
    for (const item of rendered) {
      ...
      slides.push({ id: `slide-${item.pageNumber}`, pageNumber: item.pageNumber, width: item.width, height: item.height, imageUrl: item.imageUrl, contentMode: 'pdf', blocks, imageBlocks: [] });
      item.canvas.width = 1;
