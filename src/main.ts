...
function home(): string {
  ...
  const section = `<section class="section section-tint teacher-home-section"><div class="wrap"><div class="section-heading"><div><p class="eyebrow">Pro učitele</p><h2>Co dnes potřebujete připravit?</h2></div>${link('/pro-ucitele', 'Všechny postupy pro učitele →', 'text-link')}</div><div class="teacher-home-grid">${quick.map((workflow) => `<a class="teacher-home-card" href="/pro-ucitele#workflow-${workflow.id}"><span>${esc(teacherWorkflowLabels[workflow.category])}</span><strong>${esc(workflow.title)}</strong><small>${esc(workflow.description)}</small><b>→</b></a>`).join('')}</div></div></section>`;
  return legacyHome().replace('<main id="main-content">', `<main id="main-content">${section}`);
}
