...
import { studyChatPrompts, studyJourney, studyModules, studyRecipes } from './data/guides/study-power';
...
function guideSectionContent(section: Guide['content'][number], headingTag = 'h2'): string {
  const heading = `<${headingTag}>${esc(section.heading)}</${headingTag}>`;
  ...
  const subsections = section.subsections?.map((subsection) => `<div class="guide-subsection">${guideSectionContent(subsection, 'h3')}</div>`).join('') ?? '';
  return `${heading}${paragraphs}${bullets}${subsections}`;
}

function guideStudySetup(guide: Guide): string {
  const setup = guide.studySetup;
  if (!setup) return '';
  return `<aside class="study-setup" aria-labelledby="study-setup-title"><div class="study-setup-heading"><p class="eyebrow">Studijní nastavení</p><h2 id="study-setup-title">Použijte tento průvodce aktivně</h2><span class="study-verified">Ověřeno ${esc(setup.verifiedAt)}</span></div><div class="study-setup-grid"><div><h3>Hodí se pro</h3><p>${esc(setup.bestFor)}</p></div><div><h3>Doporučené nastavení</h3><ul>${setup.recommendedSettings.map((item) => `<li>${esc(item)}</li>`).join('')}</ul></div><div><h3>Metoda</h3><p>${esc(setup.studyMethod)}</p></div><div><h3>Tip pro prompt</h3><p>${esc(setup.promptTip)}</p></div><div><h3>Častá chyba</h3><p>${esc(setup.commonMistake)}</p></div><div><h3>Vychytávka</h3><p>${esc(setup.proTip)}</p></div></div><p class="study-source">Fakta o nativní funkci: ${link(setup.officialReference.url, `${esc(setup.officialReference.label)} ${icon('external')}`)}</p></aside>`;
}

function studyCollection(): string {
  return `<section id="studium" class="section section-tint study-power-section" aria-labelledby="study-power-title"><div class="wrap"><div class="section-heading"><div><p class="eyebrow">Studium a vychytávky</p><h2 id="study-power-title">Z notebooku udělejte učební smyčku.</h2><p class="section-intro">Notebook Hub doporučuje postup, ve kterém se střídá orientace, porozumění, vybavení z paměti a ověření. Nejde o nový katalog ani o automatické hodnocení.</p></div><span class="count-stamp"><strong>${studyModules.length}</strong><small>vrstev studia</small></span></div><div class="study-journey">${studyJourney.map(([number, title, description]) => `<article><span>${number}</span><h3>${esc(title)}</h3><p>${esc(description)}</p></article>`).join('')}</div><div class="study-cycle"><div><p class="eyebrow">Doporučený cyklus Hubu</p><h3>Zdroje → mapa → chat → kartičky → kvíz → audio/video → finální kontrola</h3><p>Každý výstup je pracovní návrh. Po vizuálním nebo zvukovém formátu se vraťte k textu, citaci a vlastnímu vysvětlení.</p></div><ol>${studyModules.map(([title, role, description]) => `<li><strong>${esc(title)}</strong><span>${esc(role)} · ${esc(description)}</span></li>`).join('')}</ol></div><div class="study-chat"><div><p class="eyebrow">Chat pro aktivní učení</p><h3>Šest způsobů, jak se ptát.</h3><p>Tyto prompty jsou doporučení Hubu; v nativním Chatu si vždy zkontrolujte výběr zdrojů, styl a délku odpovědi.</p></div><div class="study-chat-grid">${studyChatPrompts.map(([title, prompt]) => `<article><strong>${esc(title)}</strong><p>${esc(prompt)}</p></article>`).join('')}</div></div><div class="study-recipes"><div><p class="eyebrow">Recepty podle času</p><h3>Vyberte malý další krok.</h3></div><div class="study-recipe-grid">${studyRecipes.map(([time, title, description]) => `<article><span>${esc(time)}</span><h3>${esc(title)}</h3><p>${esc(description)}</p></article>`).join('')}</div></div><div class="study-mistakes"><strong>Časté studijní chyby</strong><span>Pasivní poslech · příliš mnoho zdrojů · kartičky bez návratu ke zdroji · důvěra v plynulý výstup · žádné vlastní vysvětlení.</span></div></div></section>`;
}
...
function guideDetail(slug: string): string {
  ...
  return shell(`<article class="article-wrap wrap">${breadcrumb([['Průvodci', '/pruvodci'], [guideCategory(guide.category).label, `/pruvodci?kategorie=${encodeURIComponent(guide.category)}`], [guide.title, '#']])}<header class="article-header"><div class="detail-kicker">${badge(guideCategory(guide.category).label, 'light')} ${badge(guideLevelLabels[guide.level], 'soft')} <span>${esc(guide.readingMinutes)} min čtení</span></div><h1>${esc(guide.title)}</h1><p class="lead">${esc(guide.excerpt)}</p><div class="article-meta"><span>Ověřeno ${esc(guide.lastVerified)}</span><span>Pro: ${esc(guide.audience.map((item) => guideAudienceLabels[item]).join(' · '))}</span><span>Autor: ${esc(guide.author ?? 'Notebook Hub CZ')}</span><span class="spacer"></span>${favoriteButton('guide', guide.id)}${button(`${icon('share')} Sdílet`, 'share', 'icon-button')}</div>${guide.availabilityNote ? `<aside class="guide-availability"><strong>${icon('spark')} Dostupnost a účet</strong><p>${esc(guide.availabilityNote)}</p></aside>` : ''}</header>${guideStudySetup(guide)}<div class="article-layout">${guideToc(guide)}<div class="prose">${guide.content.map((section, index) => `<section id="section-${index}">${guideSectionContent(section)}</section>`).join('')}${guide.advancedSections?.length ? `<details class="guide-advanced"><summary>Podrobněji: kontrolní postupy a pokročilé varianty <span aria-hidden="true">⌄</span></summary><div class="guide-advanced-content">${guide.advancedSections.map((section, index) => `<section id="advanced-section-${index}">${guideSectionContent(section)}</section>`).join('')}</div></details>` : ''}<div class="article-callout"><strong>${icon('spark')} Kontrolní otázka</strong><p>Co v tomto postupu vyžaduje vaše vlastní ověření nebo znalost kontextu?</p></div></div></div>${relatedGuideSection(guide)}</article><section class="section section-tint"><div class="wrap"><div class="section-heading"><div><p class="eyebrow">Další čtení</p><h2>Pokračovat v knihovně</h2></div></div><div class="card-grid guide-grid">${(guide.relatedGuideIds ?? []).map((id) => guides.find((item) => item.id === id)).filter((item): item is Guide => Boolean(item)).slice(0, 3).map(guideCard).join('')}</div></div></section>`, 'pruvodci');
}
